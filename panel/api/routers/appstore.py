...
class RefreshCatalogRequest(BaseModel):
    """Request model for catalog refresh."""
    sources: Optional[List[str]] = None  # ["portainer", "linuxserver"]
...
async def refresh_catalog(request: RefreshCatalogRequest = None):
    ...
    try:
        catalog = await catalog_importer.refresh_catalog(sources)
        # Reload the app store service catalog
        await appstore_service._load_catalog()
        return {
            "success": True,
            "message": f"Catalog refreshed with {len(catalog.get('apps', []))} apps",
            "apps_count": len(catalog.get("apps", [])),
