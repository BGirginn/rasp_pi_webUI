"""
TLS helpers for Panel → Agent connections.
"""
...
def validate_cert_dates(peer_cert: Dict, now: Optional[datetime] = None) -> None:
    if not peer_cert:
        raise TLSValidationError("Missing server certificate")

    not_before = peer_cert.get("notBefore")
    not_after = peer_cert.get("notAfter")
    if not not_before or not not_after:
        raise TLSValidationError("Server certificate missing validity window")
    ...
    if now_dt < nb:
        raise TLSValidationError("Server certificate not yet valid")
    if now_dt > na:
        raise TLSValidationError("Server certificate expired")
    ...
    *,
    ...
    if expected_identities:
        if not any(identity in expected_identities for identity in identities):
            raise TLSValidationError("Server identity not allowed")

    if expected_fingerprints:
        if fingerprint.lower() not in expected_fingerprints:
            raise TLSValidationError("Server fingerprint not allowed")
