...
export default function Dashboard() {
    ...
    // Process telemetry data from either SSE or API response
    const processTelemetryData = (data) => {
        ...
        setStats({
            ...
            hostname: systemInfo.hostname || stats.hostname || 'Unknown',
            os: systemInfo.os || stats.os || 'Unknown',
            machine: systemInfo.machine || stats.machine || 'Unknown',
        })
    }

    // Load resources and alerts
    const loadOtherData = async () => {
        try {
            ...
            console.error('Failed to load resources/alerts:', err)
        }
    }
    ...
    const loadDashboardData = async () => {
        try {
            ...
            processTelemetryData(telemetryRes.data)
            await loadOtherData()
        } catch (err) {
    ...
    useEffect(() => {
        ...
        // Try SSE connection for real-time telemetry
        ...
        if (token) {
            try {
                ...
                eventSource.onerror = (err) => {
                    ...
                    // Fallback to polling
                    if (!pollingInterval) {
                        pollingInterval = setInterval(loadDashboardData, 5000)
                    }
                }
            } catch (err) {
            ...
            // No token, use polling
            pollingInterval = setInterval(loadDashboardData, 5000)
        }

        // Refresh resources and alerts every 10 seconds
        const resourceInterval = setInterval(loadOtherData, 10000)
