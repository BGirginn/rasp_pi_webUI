...
export default function Telemetry() {
    ...
    // Real history for sparklines - accumulated from dashboard updates
    const [sparklineData, setSparklineData] = useState({
        cpu: [], memory: [], disk: [], temp: []
    });

    // Update sparklines when dashboard data changes
    useEffect(() => {
        if (dashboard?.system) {
            setSparklineData(prev => ({
                cpu: [...prev.cpu.slice(-19), { value: dashboard.system.cpu_pct || 0 }],
                memory: [...prev.memory.slice(-19), { value: dashboard.system.memory_pct || 0 }],
                disk: [...prev.disk.slice(-19), { value: dashboard.system.disk_pct || 0 }],
                temp: [...prev.temp.slice(-19), { value: dashboard.system.temperature_c || 0 }],
            }));
        }
    }, [dashboard]);
    ...
    return (
        <div className="space-y-6 animate-fade-in">
            ...
            {/* Metric Cards - using real sparkline data */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <MetricCard
                    ...
                    icon="🔲"
                    color="#38bdf8"
                    data={sparklineData.cpu.length > 1 ? sparklineData.cpu : [{ value: system.cpu_pct || 0 }]}
                />
                <MetricCard
                    title="Memory"
                    ...
                    icon="💾"
                    color="#22c55e"
                    data={sparklineData.memory.length > 1 ? sparklineData.memory : [{ value: system.memory_pct || 0 }]}
                />
                <MetricCard
                    title="Disk Usage"
                    ...
                    icon="💽"
                    color="#f59e0b"
                    data={sparklineData.disk.length > 1 ? sparklineData.disk : [{ value: system.disk_pct || 0 }]}
                />
                <MetricCard
                    title="Temperature"
                    ...
                    icon="🌡️"
                    color="#ef4444"
                    data={sparklineData.temp.length > 1 ? sparklineData.temp : [{ value: system.temperature_c || 0 }]}
                />
            </div>
