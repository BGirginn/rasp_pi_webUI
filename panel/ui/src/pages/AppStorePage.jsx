...
export function AppStorePage() {
  ...
  // Refresh catalog from external sources
  const handleRefreshCatalog = async () => {
    setRefreshingCatalog(true);
    try {
      const response = await api.post('/appstore/refresh-catalog');
      alert(`Catalog refreshed! ${response.data.apps_count} apps from ${response.data.sources?.join(', ') || 'external sources'}`);
      await fetchData();
    } catch (err) {
      console.error('Failed to refresh catalog:', err);
      alert(err.response?.data?.detail || 'Failed to refresh catalog');
    } finally {
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg ${
              isDarkMode ? 'bg-purple-500/20 hover:bg-purple-500/30 text-purple-400' : 'bg-purple-100 hover:bg-purple-200 text-purple-600'
            } transition-colors disabled:opacity-50`}
            title="Import apps from Portainer & LinuxServer.io"
          >
            <Download size={18} className={refreshingCatalog ? 'animate-pulse' : ''} />
            {refreshingCatalog ? 'Importing...' : 'Import Catalog'}
          </button>
          <button
            onClick={fetchData}
