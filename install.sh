...
# Pi Control Panel - Sudoers Configuration
# This allows the pi-control service to manage systemd services

# Allow systemctl commands without password
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl start *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl stop *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl restart *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl status *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl reload *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl is-active *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl is-enabled *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl list-units *
$INSTALL_USER ALL=(ALL) NOPASSWD: /usr/bin/systemctl list-unit-files *
SUDOERS_EOF
    ...
    if visudo -cf "$sudoers_file" > /dev/null 2>&1; then
        ...
        fail "Sudoers file validation failed"
        rm -f "$sudoers_file"
    fi
}
